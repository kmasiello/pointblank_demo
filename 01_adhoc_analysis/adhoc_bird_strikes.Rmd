...
# fun fact: the object returned from scan_data is a list and html file.  You can
# save this as an rds file and retrieve it later.
# saveRDS(scan, file = "01_adhoc_analysis/reports/scan.rds")

#Or export it with 
# export_report(scan, filename = "01_adhoc_analysis/reports/tbl_scan-strikes.html")
scan <- readRDS(here::here("01_adhoc_analysis", "reports", "scan.rds"))
...
The agent is responsible for asking a set of questions (aka validations) about our data and then reporting the results.  
...
All looks good for the first validation step - there are no NULL dates.
...
Nothing critical here. I wonder if "KDEN" without the asterisk appear in the data?
```{r}
...
Cross checking flight phase information might be more important for our analysis to get correct.  Looks like from the validation report there is some clean up to be had and uniformity in definition to promote in our data entry. 
```{r}
...
Many of the climb reports should have been classified as "en route," and much of the approach data should be reclassified as "descent." Pointblank has helped us quickly find data errors and inconsistencies. We should create a clean script to reclassify flight phase information for consistency.  

### Create the informant table (Data Dictionary)
The informant table can be a rich source of documentation for our analysis. Document assumptions, sources, conventions, cleaning steps, etc. to provide important information about your data.  
```{r eval = FALSE, include = FALSE}
(informant <- strikes %>%
  create_informant(
...
1. All curated assets for this demo using `connectwidgets`: https://colorado.rstudio.com/rsc/pointblank/curation.html. 
1. Example project webpage summarizing our adhoc analysis documents https://colorado.rstudio.com/rsc/strike_project_info/. 
