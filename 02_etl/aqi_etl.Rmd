...
(agent <- response %>% create_agent(actions = action_levels(warn_at = 0.1, stop_at = 0.2)) %>% 
...
tbl <- response %>% select(Site_Name, UTC, AQI) %>% 
  arrange(desc(AQI)) %>% filter(Site_Name == first(Site_Name))
...
if(nrow(bad_air) > 0){
email <- render_connect_email(here::here("email_bad_air.rmd")) 

attach_connect_email(email, subject = "Air Quality Alert - it's bad out there", attachments = "bad_air.csv")
} else {
  email <- render_connect_email(here::here("email_good_air.rmd"))
