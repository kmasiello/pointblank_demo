...
Mask up! There's some bad air out there, baby.  Air quality index exceeds `r threshold` for `r nrow(bad_air)` readings.  These are taken over the last `r timespan` hours. See the summary agent below and data attached.  
...
```{r echo=FALSE}
#how many readings
num_bad <- nrow(bad_air)
sample_size <- min(num_bad, 6)
```
